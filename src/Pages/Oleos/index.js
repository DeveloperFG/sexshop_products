...
function Oleos() {
    ...
    useEffect(() => {
        ...
        async function loadProdutos() {
            await firebase.firestore().collection('Oleos').onSnapshot((doc) => {
                let DataUser = [];
    ...
    return (
        <div>
            {arrayProduto == 0 && nomeCliente == 'Administrador' ? (

                <div className='divAvisoCd'>
                    <small>Essa página não contém items cadastrados</small> <br></br>
                    <Link className='linkCad' to='/Cadastrar'> Cadastre aqui! </Link>
                </div>
