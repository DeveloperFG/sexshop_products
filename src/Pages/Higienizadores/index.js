...
import { Card, DivCards } from '../../components/Cards/styles'
...
import './../Oleos/oleos.css'


function Higienizadores() {
    ...
    let { arrayProduto, setArrayProduto, select, setSelect } = useContext(CartContext);


    useEffect(() => {

        async function loadProdutos() {
            await firebase.firestore().collection('Higienizadores').onSnapshot((doc) => {
                let DataUser = [];
    ...
    return (
        <div>
            {arrayProduto == 0 ? (
                ...
                <div className='divAvisoCd'>
                    <p>Nenhum item disponível </p> <br></br>
                </div>
