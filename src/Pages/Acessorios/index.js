...
function Acessorios() {
    ...
    let { setImagemModal } = useContext(CartContext);
    ...
    useEffect(() => {
        ...
        async function loadProdutos() {
            await firebase.firestore().collection('Acessorios').onSnapshot((doc) => {
                let DataUser = [];
    ...
    return (
        <DivCards onClick={closeSidebar} style={{ justifyContent: lista.length < 3 ? 'space-around' : 'space-between' }}>
            {lista.map((item) => {
                return (
                    <Card key={item.id}>
                        {item.type == 'array' ? <Link to={`/ItemSlide/${item.id}/${item.titulo}/${item.categoria}`} onClick={() => salvarLocal(item)}> <img className='imgItems' src={item.img[0]} alt={item.titulo} /> </Link> :
